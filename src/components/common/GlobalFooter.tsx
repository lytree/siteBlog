import { type DefineComponent, defineComponent, type PropType } from 'vue';
import ColorSwitch from "@/components/common/ColorSwitch.tsx";


export default defineComponent({
    name: "GlobalFooter",
    inheritAttrs: false,
    setup(props) {
        return () => (
            <footer class="z-10 md:p-[16px] w-full border-top text-[14px] whitespace-nowrap">
                <div class="my-0 mx-auto max-w-[var(--layout-max-width)] text-center">
                    <p class="text-sm m-0 font-medium leading-32px transition: color 0.25s;">
                        <slot name="copyright">
                            Copyright © 2019 - {new Date().getFullYear()}
                            <ColorSwitch class="h-32px w-32px"></ColorSwitch>
                        </slot>
                    </p>
                    <p class="text-sm m-0 font-medium leading-32px transition: color 0.25s;">
                        <slot name="message">
                            <img class="inline-block"
                                src="data:image/png;base64,"
                                alt="" width="15">公安备案信息:<a
                                    href="http://www.beian.gov.cn/portal/registerSystemInfo?recordcode=41018402000536">
                                    豫公网安备41018402000536号
                                </a>备案信息:<a href="https://beian.miit.gov.cn/ ">豫ICP备2021004086号-1</a>
                            </img>
                        </slot>
                    </p>
                </div>
            </footer>
        );
    },
})